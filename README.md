...
- অফলাইন লগইন পেজ — **একই ফর্মে সবার প্রবেশ**: শিক্ষার্থী **ইউজারনেম (বা মোবাইল নম্বর)** ও ৪–৬ সংখ্যার PIN দিয়ে, আর শিক্ষক/এডমিন/পেমেন্ট কাউন্টার নিজের ইউজারনেম (`teacher.apc` / `admin.apc` / `payment.apc`) ও পাসওয়ার্ড দিয়ে — সেশন আগে জমা হয়, তাই প্যানেল দ্বিতীয় ফর্ম ছাড়াই খোলে
...
- `config.js`-এ শিক্ষার্থীর `DEFAULT_PIN = '123123'`। এটি শুধু নতুন নিবন্ধন ও PIN-reset-এর খালি ঘরের fallback। এডমিন, শিক্ষক ও পেমেন্ট ডেস্ক এই PIN ব্যবহার করে না — তাদের ইউজারনেম `admin.apc` / `teacher.apc` / `payment.apc` এবং পাসওয়ার্ড `Apc@2026`। নিবন্ধনে নিজের PIN বেছে নেওয়া যায়। পুরোনো custom PIN স্বয়ংক্রিয়ভাবে reset করা হয় না।
...
- চার ধরনের ব্যবহারকারীই লগইন করেন `index.html`-এর একই ফর্ম দিয়ে: শিক্ষার্থী (ইউজারনেম/মোবাইল + PIN), শিক্ষক, এডমিন ও পেমেন্ট কাউন্টার (নিজের ইউজারনেম + পাসওয়ার্ড)। ট্যাব, শর্টকাট লিংক বা আগে থেকে লেখা পাসওয়ার্ড কিছুই নেই।
...
ডিভাইসেই থাকবে: শিক্ষার্থীর PIN, স্টাফ পাসওয়ার্ড, সেশন ও ইউজারনেম ইনডেক্স। এগুলো `LOCAL_ONLY` — Firebase-এ তোলা যাবে না। নতুন নোটিশ, রুটিন ক্লাস ও Student ID এখন মুছে-আবার-যোগ করলেও একই আইডিতে ধাক্কা খায় না। নতুন ফি রেকর্ডে `recordedAt` থাকে, তাই একই দিনের আদায় সময় অনুযায়ী সাজানো যায়।
