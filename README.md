...
- এডমিন প্যানেল — ইউজারনেম `admin.apc` ও পাসওয়ার্ড দিয়ে প্রবেশ (`admin.html`)

## এডমিন প্যানেল

`admin.html` খুললে এডমিন এন্ট্রি স্ক্রিন দেখা যায়। ইউজারনেম **`admin.apc`**, পাসওয়ার্ড **`Apc@2026`**। এক ক্লিকে ঢোকা বা আগে থেকে লেখা পাসওয়ার্ড নেই। “এই ডিভাইসে মনে রাখুন” চেক থাকলে সেশন ৯০ দিন থাকে। শিক্ষার্থী লগইন পেজে আলাদা কোনো স্টাফ ট্যাব বা লিংক নেই, কিন্তু একই লগইন ফর্মে `admin.apc` ও পাসওয়ার্ড দিলে সরাসরি এই প্যানেল খুলে যায় (সেশন আগেই জমা হয়); `admin.html` সরাসরি খুলেও লগইন করা যায়। প্যানেল থেকে **লগআউট** করলে সেশন মুছে সোজা ওই একই লগইন পেইজে ফেরে।
...
- **একই লগইন পেজ থেকেও প্রবেশ** — শিক্ষার্থী লগইন ফর্মে কাউন্টারের ইউজারনেম **`payment.apc`** ও পাসওয়ার্ড **`Apc@2026`** দিলে সেশন জমা হয়ে সরাসরি ডেস্ক খোলে (`payment.html` নিজেও এন্ট্রি ফর্ম দেখায়)। ভুল হলে বাংলা ত্রুটি-বার্তা। “এই ডিভাইসে মনে রাখুন” চেক থাকলে সেশন ৯০ দিন, না হলে ট্যাব-সেশন। সেশন থাকলে ডেস্ক সরাসরি খোলে। **লগআউট** করলে সেশন মুছে সোজা লগইন পেইজে (`index.html`) ফেরে।
...
এডমিন প্যানেল: `http://localhost:4173/admin.html` — ইউজারনেম `admin.apc`, পাসওয়ার্ড `Apc@2026`। শিক্ষক: `teacher.apc`। পেমেন্ট ডেস্ক: `payment.apc`। তিনটিতেই প্রথম পাসওয়ার্ড `Apc@2026`।
...
`teacher.html` খুলুন, অথবা শিক্ষার্থী লগইন পেজেই `teacher.apc` ও পাসওয়ার্ড দিয়ে ঢুকুন (`admin.apc` দিলে এডমিন প্যানেল, `payment.apc` দিলে পেমেন্ট ডেস্ক)। এডমিন প্যানেল → আরও → শিক্ষক প্যানেল পথও আছে। ইউজারনেম **`teacher.apc`**, পাসওয়ার্ড **`Apc@2026`**। এক ক্লিকে প্রবেশ নেই। ডেটা এই ব্রাউজারে থাকে; এটি সার্ভার-ভিত্তিক শিক্ষক অ্যাকাউন্ট নয়।
...
- একই পরিচিত default পাসওয়ার্ড নিরাপদ production credential নয়। অনলাইন চালুর আগে secure authentication, hashed passwords, প্রথম লগইনে পাসওয়ার্ড পরিবর্তন এবং server-side immutable-number enforcement প্রয়োজন। Local storage হাতে বদলানো/মুছে ফেলা এই demo দিয়ে আটকানো যায় না।
...
- শিক্ষার্থী নিজে রেজিস্ট্রেশন করে। স্টাফ লগইন এই তিনটি: এডমিন `admin.apc`, শিক্ষক `teacher.apc`, পেমেন্ট `payment.apc`। তিনটিতেই প্রথম পাসওয়ার্ড `Apc@2026`।
