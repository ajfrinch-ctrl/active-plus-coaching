...
- **সংক্ষিপ্ত ড্যাশবোর্ড** — বর্তমান তারিখ, মোট শিক্ষার্থী, চলমান ক্লাস; নিচে **অনুমতি-ভিত্তিক বিভাগ গ্রিড** (বড় আইকন + স্পষ্ট লেবেল: শিক্ষার্থী, হিসাব, রিপোর্ট, নোটিশ, রুটিন, পরীক্ষা, ক্লাস সেটিংস, অ্যাপ কন্ট্রোল — কেবল Role-এর অনুমোদিত বিভাগ), এরপর ফি গ্রহণের দ্রুত বাটন ও **অর্থ ব্যবস্থাপনা ও ফি সারাংশ কার্ড** — আজকের আদায় (লেনদেন সংখ্যাসহ), এই মাসের আদায়, চলতি মাসের বকেয়া ও সর্বমোট আদায়; "বিস্তারিত হিসাব" চাপলে হিসাব পেজে যায়। অনুমোদনের অপেক্ষমাণ সারাংশ, নোটিশের সংখ্যা, পুনরাবৃত্ত শিক্ষার্থী/ক্লাস তালিকা ও অ্যাপের বিস্তারিত status কার্ড নেই — অনুমোদন Manager প্যানেলের কাজ (নিচে দেখুন)।
- **৫টি মোবাইল নেভিগেশন** — ড্যাশবোর্ড, শিক্ষার্থী, হিসাব, রুটিন ও আরও; প্রতিটিতে AI-generated আইকন + লেবেল, নির্বাচিত ট্যাবের আইকন সামান্য বড় হয়ে গ্লোসহ স্থায়ীভাবে হাইলাইট থাকে। **আরও**-এর মধ্যে পরীক্ষা, নোটিশ, রিপোর্ট জেনারেটর, শিক্ষার্থী অ্যাপ ও ক্লাস সেটিংস; প্রতিটি পেজ থেকে আরও-তে ফেরার বাটন আছে। এই পেজগুলোতেও ফুটারের আরও বাটন সক্রিয় থাকে। আলাদা desktop sidebar বা desktop navigation নেই; বড় স্ক্রিনেও একই মোবাইল নেভিগেশন থাকে।
...
### এডমিন প্যানেলে রোল-ভিত্তিক অ্যাক্সেস

Admin প্যানেল কোনো নতুন রোল বা স্টোরেজ কী তৈরি করে না। এটি রেপোতে আগে থেকে থাকা
রোল-সীমানা (`firestore.rules`, `functions/index.js`, আলাদা `manager.html`) পড়ে
সেটিকেই UI-তে প্রয়োগ করে — `js/admin-permissions.js`:

| উৎস | Admin-এর ক্ষমতা |
| --- | --- |
| `firestore.rules` | রিপোর্ট, সেটিংস, নোটিশ, রুটিন, ট্রানজেকশন, পরীক্ষার রেকর্ড — পড়া/লেখা |
| `functions/index.js` | শিক্ষার্থী approve/reject ও পরীক্ষা publish **শুধু Manager** |
| `manager.html` / `teacher.html` / `payment.html` | ওই রোলগুলোর নিজস্ব প্যানেল |

ফলে Admin-এর অনুমোদিত বিভাগ: **ড্যাশবোর্ড, শিক্ষার্থী (দেখা/সম্পাদনা), হিসাব ও ফি গ্রহণ,
রিপোর্ট, নোটিশ, রুটিন, পরীক্ষা, ক্লাস সেটিংস, শিক্ষার্থী অ্যাপ কন্ট্রোল**। Role-এর বাইরের
সবকিছু UI থেকে সরিয়ে দেওয়া হয় — শুধু লুকানো নয়, DOM থেকে মুছে ফেলা হয়:

- অনুমোদনের অপেক্ষমাণ শর্টকাট/সারাংশ (Manager), শিক্ষার্থীর approve-reject অ্যাকশন
- পরীক্ষা প্রকাশ/প্রত্যাখ্যান কন্ট্রোল (Manager) — Admin শুধু তালিকা ও ফলাফল রিপোর্ট দেখে
- শিক্ষক প্যানেল ও পেমেন্ট রিসিভ প্যানেলের লিংক (Teacher / Payment রোল)

প্রয়োগ দুই স্তরে:

1. **UI visibility** — `data-admin-cap="…"` চিহ্নিত প্রতিটি মেনু, কার্ড, শর্টকাট ও
   `data-view-panel` সেকশন `enforceCapabilities()` মুছে দেয়; বটম বার, আরও-মেনু ও
   ড্যাশবোর্ড গ্রিড `js/admin-panel-ui.js` সরাসরি অনুমোদিত এন্ট্রি থেকে তৈরি করে, তাই
   অননুমোদিত কিছু কখনোই রেন্ডার হয় না।
2. **Access control** — `setView()`/`navigate()` প্রতিবার `Access.allowsView()` যাচাই করে।
   `admin.html#finance`-এর মতো ডিপ-লিংকও শুধু তখনই খোলে যখন Role-এর অনুমতি থাকে; বাকিগুলো
   প্রথম অনুমোদিত ট্যাবে ফেরত যায় (কোনো unauthorized কনটেন্ট লিক হয় না)।

### এডমিন প্যানেলের আইকন সিস্টেম
...
css/
  ...
  admin-panel-ui.css    Admin Panel shell: permission grid, bottom-bar icons/labels,
                        typography ও contrast (loaded last on admin.html)
assets/fonts/
...
assets/icons/
  ...
  glass/*.png          shared water-drop glass icon family (home, routine, finance, …)
  admin/*.png          Admin Panel icon set — dashboard, users, finance, payment,
                       reports, notices, classes, app, exams (AI-generated soft-3D glass)
js/
