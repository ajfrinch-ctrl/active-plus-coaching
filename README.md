...
## ডেমো এডমিন প্যানেল (এক ক্লিকে)

লগইন স্ক্রিনের **“ডেমো এডমিন প্যানেল — এক ক্লিকে”** বোতামে চাপলেই প্যানেল খুলে যায় — আলাদা অ্যাকাউন্ট, PIN বা সার্ভার লাগে না। ডেমো সেশন চালু থাকলে শিক্ষার্থী টপবারে একটি স্লাইডার আইকনও দেখা যায়, সেখান থেকেও ঢোকা যায়।

| সেকশন | কী করা যায় |
| --- | --- |
| সারসংক্ষেপ | মোট পরীক্ষা, জমা, বাকি অনুমোদন, নোটিশ — এক নজরে; JSON এক্সপোর্ট ও রিসেট |
...
- **কোথায় থাকে:** সবকিছু `localStorage`-এর `active-plus-admin-v1` কী-তে। `js/config.js` বা রিপো-র কোনো ফাইল বদলায় না; লুকানো/ওভাররাইড সব ডিভাইস-লোকাল।
- **উত্তর নিরাপত্তা:** এডমিন সঠিক অপশনটি প্লেইন টেক্সটে লেখে, সেভ করার সময় `js/exam-hash.js` দিয়ে হ্যাশ হয় — শিক্ষার্থীর কাছে যাওয়া তালিকায় উত্তর থাকে না (ইনডেক্সটা শুধু এডমিনের ম্যাপে থাকে)।
- **মডুল বিন্যাস:** `js/admin-data.js` (ডেটা লেয়ার) + `js/admin.js` (প্যানেল UI) + `js/notices.js` (প্রকাশিত নোটিশ রেন্ডার) + `css/admin.css`; `index.html`-এ শুধু ট্যাব ও `#adminBody` হুক।
- **সীমাবদ্ধতা:** এটি ডেমো — কোনো প্রমাণিকরণ নেই, তাই এই ডিভাইসে যে কেউ প্যানেলে ঢুকে লোকাল ডেটা দেখতে/বদলাতে পারবে। আসল এডমিন প্যানেলে লগইন, ভূমিকা (role) ও সার্ভার-সাইড গ্রডিং লাগবে।

## সম্পর্কিত: `ssc-prep/` (আলাদা assessment মডিউল)

MCQ প্র্যাকটিস, টাইড মডেল টেস্ট/লাইভ এক্সাম, সৃজনশীল প্রশ্ন ও পারফরম্যান্স ড্যাশবোর্ড — Tailwind CSS + vanilla JS + JSON দিয়ে `ssc-prep/` ফোল্ডারে একটি স্বয়ংসম্পূর্ণ প্রোটোটাইপ আছে। এটি এই PWA-এর অংশ নয় (আলাদা entry point, আলাদা localStorage key), তাই মূল অ্যাপের cache বা UI-এর সাথে সংঘর্ষ হয় না। চালানোর নিয়ম ও JSON চুক্তি `ssc-prep/README.md`-এ।
