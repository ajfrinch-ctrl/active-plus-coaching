...
- **আমার কোর্স (শিক্ষার্থী পোর্টাল)** — ক্যাটাগরি টাইল সবসময় সারিতে **দুটি করে অর্ধেক প্রস্থে** (৩টি হলে ২+১ ফুল, ৪টি হলে ২+২); শিক্ষকের দেওয়া কাজের কার্ড ও ডেমো কোর্স তালিকা **চাপলেই তথ্য খোলে** (সংক্ষিপ্ত ভিউতে ধরন/অবস্থা/শেষ সময়, খুললে বিস্তারিত-মেটা, শিক্ষক ও অ্যাকশন); কার্ডের কোণায় ঘোরানো চেভরন খোলা/বন্ধ বোঝায়
...
এডমিন প্যানেলের বাইরে আলাদা, খুব হালকা একটি কাউন্টার পেজ — ইউনিক ইউসার আইডি ও PIN দিয়ে প্রবেশ; কোনো নেভিগেশন, রিপোর্ট বা ম্যানেজমেন্ট নেই। ধারাবাহিকতা: **লগইন → সার্চ → সংক্ষিপ্ত তথ্য → পেমেন্ট → রসিদ**।

- **প্রবেশাধিকার** — ইউনিক ইউসার আইডি `APC-PAY-001`, ডিফল্ট PIN **১২৩১২৩** (দুটোই ডেমো ফর্মে আগেই লেখা থাকে)। ভুল আইডি/PIN-এ বাংলা ত্রুটি-বার্তা; "এই ডিভাইসে মনে রাখুন" চেক করা থাকলে সেশন ৯০ দিন মনে থাকে, না হলে ট্যাব-সেশন পর্যন্ত। টপবারের **লক আইকনে** বর্তমান PIN যাচাই করে নতুন PIN (৪–৬ সংখ্যা, বাংলা সংখ্যাও চলে) দুইবার লিখে বদলানো যায় — অ্যাকাউন্ট `activePlus.paymentAccount.v1`, সেশন `activePlus.paymentSession.v1` কী-তে সংরক্ষিত; ডেমো হিসেবে সাধারণ localStorage-এ সাদামাটা ভাবে রাখা, সার্ভার যুক্ত হলে এই কী-গুলোই API অ্যাকাউন্ট দিয়ে বদলাবে। **প্রস্থান** আইকনে লগআউট।

- **সার্চবার** — শিক্ষার্থীর নাম, মোবাইল নম্বর, Student ID **অথবা অভিভাবকের মোবাইল নম্বর** দিয়ে খোঁজা যায় (বাংলা সংখ্যা ও `+880` ফরম্যাটসহ)। এডমিন প্যানেলের ফি-সার্চেও এখন অভিভাবকের নম্বর কাজ করে।
- **সংক্ষিপ্ত তথ্য** — নির্বাচন করলেই শ্রেণি-বিভাগ, শিক্ষার্থী ও অভিভাবকের মোবাইল, সর্বশেষ পেমেন্ট, নির্ধারিত মাসিক ফি, চলতি মাসে পরিশোধ ও বকেয়া; সঙ্গে কোন নম্বরে রসিদ যাবে তার হিন্ট।
- **পেমেন্ট** — এডমিন প্যানেলের ফর্মের মতোই (ফি ধরন, মাস, দ্রুত পরিমাণ চিপ, মাধ্যম, Trx ID, নোট); একই `financeRepository` localStorage contract-এ সংরক্ষণ — এডমিন প্যানেলের হিসাব, লেজার ও রিপোর্টে সঙ্গে সঙ্গে দেখা যায়। সংরক্ষণ ব্যর্থ হলে রসিদ হয় না।
- **হোয়াটসঅ্যাপে রসিদ (এক ক্লিকে)** — রসিদ সেভ হলেই মোডালে "হোয়াটসঅ্যাপে পাঠান" বাটন: রসিদটি অফলাইন canvas-এ ছবি (PNG) আঁকা হয় এবং Web Share API-তে ফাইলসহ শেয়ার মেনু খোলে — WhatsApp বেছে নিলেই শিক্ষার্থীর নম্বরে ছবি চলে যায়। শেয়ার সাপোর্ট না থাকলে ছবিটি ডাউনলোড হয়ে শিক্ষার্থীর নম্বরে `wa.me` চ্যাট (৮৮০ প্রিফিক্সসহ) খুলে যায়, পেমেন্টের সংক্ষিপ্ত টেক্সটসহ; শিক্ষার্থীর নম্বর না থাকলে অভিভাবকের নম্বর ব্যবহৃত হয়। "রসিদ PDF ডাউনলোড" বাটনে আগের মতোই ব্র্যান্ডেড PDF।
...
- **সহায়ক উপকরণ খুলুন (শিক্ষার্থী কোর্স কার্ড)** — লিংকে চাপলে আর খালি নতুন ট্যাব নয়: পপ-আপে কাজের শিরোনাম ও ফাইলের নাম দেখা যায়, **ডাউনলোড করুন** বাটনে ফাইলটি সরাসরি ডাউনলোড হয় (fetch + অ্যাপের নিজস্ব downloadBlob, অফলাইনেও); সরাসরি ডাউনলোড ব্লক থাকলে স্বয়ংক্রিয়ভাবে নতুন ট্যাবে খোলে। ডেমো উপকরণ এখন অ্যাপের canvas→PDF পাইপলাইনে তৈরি ব্র্যান্ডেড **PDF** (`assets/demo-study-notes.pdf`, sw.js ক্যাশে); আগের ডেমো ডেটার পুরনো .txt লিংক প্রথম লোডেই PDF-এ রূপান্তরিত হয়। ফন্টে না থাকা ↗ তীর-চিহ্নের বদলে ইনলাইন SVG।
...
- শিক্ষক অংশে ৫টি নমুনা পরীক্ষা/বাড়ির কাজ/সাজেশন/ক্লাস, নম্বর, জমার অবস্থা ও উপস্থিতি। সহায়ক লিংক `assets/demo-study-notes.txt`-এর প্রকৃত bundled Bangla নোট খুলবে; অফলাইনেও cache হয়।
