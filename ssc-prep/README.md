...
মূল চারটি ফিচার, সাথে লোকাল প্রশ্নব্যাংক ওভারলার সহায়তা:
...
5. **লোকাল এডিট পড়া হয়** — প্রশ্নব্যাংক/সেট তৈরি-সম্পাদনা হয় মূল অ্যাপের **“শিক্ষার্থী এপ ম্যানেজমেন্ট”** প্যানেলে (`../?manage=1&tab=ssc`); এই অ্যাপের নিজস্ব এডমিন UI ইচ্ছাই নেই, যাতে এডিটরের দ্বৈত সংস্করণ না থাকে। মূল অ্যাপ পাশে সার্ভ করা থাকলে হেডারে এক ক্লিকে লিঙ্ক দেখা যায় (লোকাল এডিট থাকলে তার গায়ে amber ডট)।
...
| `ssc-prep-bank-v1` | ডিভাইস-লোকাল প্রশ্নব্যাংক ওভারলে — এখানে শুধু পড়া হয়, লেখে ম্যানেজমেন্ট প্যানেল (নিচে দেখো) |

## প্রশ্নব্যাংক ম্যানেজমেন্ট (এডমিন)

এই অ্যাপে এডমিন স্ক্রিন নেই — সব ম্যানেজমেন্ট মূল PWA-এর **শিক্ষার্থী এপ ম্যানেজমেন্ট** প্যানেলের **SSC প্রস্তুতি** ট্যাবে (`../js/admin-ssc.js`): MCQ ব্যাংক · সৃজনশীল · টেস্ট সেট · ফলাফল পর্যবেক্ষণ · JSON ইমপোর্ট/এক্সপোর্ট। দুটো অ্যাপ একই origin শেয়ার করায় প্যানেল এখানেই বসে, আলাদা সার্ভার লাগে না।

- **একটাই নিয়ম:** `data/questions.json` কখনো লেখা হয় না। সব পরিবর্তন `ssc-prep-bank-v1` কী-এ *প্যাচ* হিসেবে থাকে (`{ mcq: { id: {...} }, cq: {}, sets: {} }`), boot-এর আগে `mergeBank()` সেগুলো মিশিয়ে দেয় — এই অ্যাপ ওভারলে শুধু পড়ে। নতুন এন্ট্রি = পূর্ণ অবজেক্ট, এডিট = শুধু বদলানো ফিল্ড (`minimalDiff`), লুকানো = `{ removed: true }`।
- **যাচাই একই জায়গায়:** প্যানেল সেভ করার আগে `validateMcq` / `validateCq` / `validateSet` চালায় — ফাংশনগুলো এই অ্যাপের `js/exam-logic.js` থেকেই import করা, তাই নিয়ম দুইরকম হতে পারে না।
- **ইমপোর্ট:** যে key পাঠাবে (`questions.mcq` / `questions.cq` / `sets`) সেই তালিকাই চূড়ান্ত ধরা হয় — বাদ পড়া শিপ করা এন্ট্রি লুকানো হয়, মুছে যায় না।
- **রিসেট:** “শিপ করা JSON-এ ফেরত” ওভারলে key সরিয়ে দেয়; শিক্ষার্থীর ফলাফল/খসড়া অটুট থাকে। মূল প্যানেলের “দুটো স্কোপই রিসেট” একসাথে দুটো key-ই মুছে দেয়।
- **শিপ করা এন্ট্রি লুকালে** সেটি প্যানেলের “লুকানো এন্ট্রি” স্ট্রিপে থাকে (যেকোনো সময় চালু করা যায়), আর এই অ্যাপে দেখা যায় না — `data/questions.json`-এর কোনো লাইন বদলায় না।
- **সময়সীমা নেই এমন সেট:** `durationMin: 0` হলে রানারে ঘড়ি চলে না (∞ দেখায়) এবং auto-submit হয় না — শুধু `model`/`live` সেটে সময় বাধ্যতামূলক (ম্যানেজমেন্ট প্যানেলও সেটাই মানে)।

শিক্ষার্থীর দিকে: ট্যাব বন্ধ করলেও খসড়া থাকে — তালিকার কার্ডে **“চালিয়ে যাও (৩/৮)”** দেখাবে; আবার ঢুকে সময় শেষ থাকলে সঙ্গে সঙ্গে auto-submit হয়। “হিস্ট্রি মুছুন” ফলাফল + CQ + খসড়া সব মুছে দেয় (থিম থাকে)।

## টেস্ট

```bash
node --test ssc-prep/tests/exam-logic.test.mjs   # ২৪টি কেস: ডেটা চুক্তি, গ্রডিং, উইন্ডো, shuffle, summarize, ব্যাংক ওভারলে
```
...
- প্রশ্নব্যাংক নমুনা হিসেবে ছোট (১৪ MCQ, ৩ CQ, ৩ সেট); `data/questions.json`-এ নতুন এন্ট্রি যোগ করলে কিংবা ম্যানেজমেন্ট প্যানেল থেকে সেভ করলেই UI নিজে থেকেই ফিল্টার, প্যালেট ও ড্যাশবোর্ড হালনাগাদ করে।
- ম্যানেজমেন্ট লিঙ্কটি দেখা যায় শুধু যখন মূল অ্যাপটা পাশে থাকে (`../manifest.json` HEAD সফল)। `ssc-prep/` আলাদাভাবে হোস্ট করলে লিঙ্কটি লুকিয়ে যায়, অ্যাপ তবুও হুবহু চলে।
