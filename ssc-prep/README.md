...
মূল চারটি ফিচার, সাথে একটি ডেমো এডমিন প্যানেল:
...
5. **ডেমো এডমিন প্যানেল** — হেডারের **“এডমিন”** বোতামে এক ক্লিকে ঢোকে (কোনো লগইন নেই): প্রশ্নব্যাংক ও সেট তৈরি/সম্পাদনা, লুকানো এন্ট্রি, ফলাফল পর্যবেক্ষণ, JSON ইমপোর্ট-এক্সপোর্ট।
...
| `ssc-prep-bank-v1` | এডমিন প্যানেলের ডিভাইস-লোকাল ওভারলে (নিচে দেখো) |

## ডেমো এডমিন প্যানেল

হেডারের **এডমিন** বোতাম — এক ক্লিকে, কোনো PIN/লগইন ছাড়াই। ভেতরে পাঁচটি ট্যাব: MCQ ব্যাংক · সৃজনশীল · টেস্ট সেট · ফলাফল পর্যবেক্ষণ · JSON।

- **একটাই নিয়ম:** `data/questions.json` কখনো লেখা হয় না। সব পরিবর্তন `ssc-prep-bank-v1` কী-এ *প্যাচ* হিসেবে থাকে (`{ mcq: { id: {...} }, cq: {}, sets: {} }`), রেন্ডারের আগে `mergeBank()` সেগুলো মিশিয়ে দেয়। নতুন এন্ট্রি = পূর্ণ অবজেক্ট, এডিট = শুধু বদলানো ফিল্ড (`minimalDiff`), লুকানো = `{ removed: true }`।
- **লুকানো বনাম মুছে ফেলা:** শিপ করা এন্ট্রি লুকালে সেটি “লুকানো এন্ট্রি” স্ট্রিপে থেকে যায় (যেকোনো সময় চালু করা যায়); এডমিন-বানানো এন্ট্রি মুছে ফেললে প্যাচটাসহ সরে যায়।
- **যাচাই:** সেভ করার আগে `validateMcq` / `validateCq` / `validateSet`, আর পুরো ব্যাংকের জন্য `validateBank` — ত্রুটি থাকলে লাল নোটিশ, কোনো আংশিক সেভ নেই।
...
- **রিসেট:** “শিপ করা JSON-এ ফেরত যাও” ওভারলে key সরিয়ে দেয়; শিক্ষার্থীর ফলাফল/খসড়া অটুট থাকে।
- **সময়সীমা নেই এমন সেট:** `durationMin: 0` হলে রানারে ঘড়ি চলে না (∞ দেখায়) এবং auto-submit হয় না — শুধু `model`/`live` সেটে সময় বাধ্যতামূলক।
